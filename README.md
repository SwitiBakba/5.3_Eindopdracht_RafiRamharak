...
In Program.cs kan het alfabet daarna worden toegevoegd:
...
Hiermee is ondersteuning voor het Russisch-cyrillische alfabet toegevoegd.

Externe validator toevoegen

Een externe validator moet IExternalPasswordVerifier implementeren.
...
De factory kiest welke generation strategy gebruikt moet worden. Hierdoor hoeft `PasswordService` niet zelf te weten welke concrete strategy nodig is.
...
public class ExampleVerifier : IExternalPasswordVerifier
{
    ...
    {
        // Hier kan een externe API worden aangeroepen.
        return true;
    }
}
```
...
```csharp
passwordService.AddExternalVerifier(new ExampleVerifier());
```

## Logging

Logging wordt geregeld via:

- `ILogger`
- `ConsoleLogger`
- `LogLevel`

Er zijn drie logniveaus: `None`, `Warnings` en `All`.

Als logging uit moet staan, kan `ConsoleLogger` worden gebruikt met `LogLevel.None`:

```csharp
ILogger logger = new ConsoleLogger(LogLevel.None);
```

Daarom is een aparte `NullLogger` niet nodig.

## Tests

De solution bevat een apart testproject: `PassTheWord.Tests`.
...
## Opmerking over security

De originele implementatie gebruikte een `Span<char>` buffer.
In deze refactor is gekozen voor string-gebaseerde verwerking, omdat de focus van deze opdracht ligt op leesbaarheid, structuur, design patterns en uitbreidbaarheid.